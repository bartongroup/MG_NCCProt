...
@fig-cormat-bat, @fig-pca-bat and @fig-clustering-bat shows that strong batch effects exist in data. These batches are due to separate MS/MS runs. For further processing batch effects are removed using *limma's* function `removeBatchEffects`, which uses linear modelling to estimate and remove the unwanted variation. It fits a linear model to the data where the batches (or other confounding factors) are used as covariates. The function then subtracts the estimated batch effects from the original data. The function was used with the following parameters:

```
x_corrected <- limma::removeBatchEffect(x, batch = "batch", design = design_matrix)
```

where `x` is data matrix and `design_matrix` is a design matrix crated using the formula `~ treatment + time_point`. Because there is only one negative sample in a given time point, treatment and batch, the coefficients for it cannot be established. Therefore, negative samples are not included in batch correction.

Batch-corrected data are used for downstream analysis.
...
```{r}
#| label: fig-da-full
#| fig-cap: Differential abundance plots of the full model `~ treatment + time_point`. Black points indicate proteins with significant differential abundance, defined as FDR < 0.05 and |logFC| > 1. A. Volcano plot. B. MA plot.
#| fig-width: 6
#| fig-height: 10
...
Note that @fig-prots-da-full shows all proteins changing in IP, but some of them are also differentially abundant in the Input (see @fig-prots-da-input-full). When input-changing proteins are removed, we are only left with proteins shown in @tbl-full-ip-drb and @tbl-full-ip-tpl.

```{r}
#| label: tbl-full-ip-drb
#| tbl-cap: Proteins that are differentially abundant for DRB treatment in IP, but not in input.
tar_read(da_ip_only_drb) |> 
  select(Gene = gene_symbols, logFC, FDR, Description = protein_names) |> 
  gt() |> 
...
#| tbl-cap: Proteins that are differentially abundant for TPL treatment in IP, but not in input.
tar_read(da_ip_only_tpl) |> 
  select(Gene = gene_symbols, logFC, FDR, Description = protein_names) |> 
  gt() |> 
  fmt_scientific(FDR, rows = FDR < 1e-3, n_sigfig = 2) |> 
  fmt_number(FDR, rows = FDR >= 1e-3, n_sigfig = 2) |> 
...
There are only two proteins with interaction effects, shown in @fig-prots-interaction. The interaction is weak and the only obvious thing is the drop at DRB_2h in SATB. Warning: there are data gaps here and the picture is incomplete.

```{r}
#| label: fig-prots-interaction
#| fig-cap: Batch-corrected normalised log-abundance of proteins showing interaction between treatment and time point.
#| fig-width: 6
#| fig-height: 4
tar_read(fig_prots_int_e1_ip)
```
...
#| fig-cap: Differential abundance for a selection of contrasts. Black points indicate proteins with significant differential abundance, defined as FDR < 0.05 and |logFC| > 1. A. Volcano plot. B. MA plot.
#| fig-width: 8
#| fig-height: 15
...
#| fig-cap: Differential abundance plots of the full model `~ treatment + time_point + batch`. Black points indicate proteins with significant differential abundance, defined as FDR < 0.05 and |logFC| > 1. A. Volcano plot. B. MA plot.
#| fig-width: 6
#| fig-height: 10
...
plot_grid(g1, g2, labels = c("A", "B"), ncol = 1, rel_heights = c(4, 6))
```
...
#| fig-cap: Normalised log-abundance of proteins significantly changing for treatments A. DRB and B. TPL.
#| fig-width: 8
#| fig-height: 12
...
Perhaps a different experimental design could be considered. Rather than attempting to encompass multiple treatments, time points, IP, input and negative controls, a simpler approach could prove more beneficial. With the capacity of a 10-plex TMT, a potential design could allocate one MS run to accommodate 5 DRB and 5 DMSO replicates, and another MS run for 5 TPL and 5 DMSO replicates (just nascent for simplicity). This would likely circumvent batch effects and improve statistical power of the experiment.
